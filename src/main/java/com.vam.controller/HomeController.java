...
/**  일자와 시간을 표기하는 포맷 양식뿐만 아니라 보통 문자열을 일자나 시간으로 또는 아래와 같이 일자나 시간을 문자열로 바꾸는 기능을 제공
     ...
     구문 분석(문자를 날짜로 변환)등의 목적으로 주로 사용된다.   
*/
...
public class HomeController {
     ...
     */
    @RequestMapping(values = "/", method = RequestMethod.GET) // value = URL 값, method = Http Request 메소드 값 (Spring4.3이후 GET, POST는 같이 쓸 수 있다.)
    /**@RequestMapping
    ...
    public String home(Locale locale, Model model) {
        ...
        model.addhAttribute("serviceTime", formattedDate);

        return "home"
    }

    /**@RequestParam
      * 1.사용자가 원하는 매개변수에 값을 매핑하기위해 사용한다.
            ex1)
                @PostMapping("/member")
      ...
      * 2.url 경로를 변수화하여 사용할 수 있도록 해준다.
            ex)
                ...
                => RequestMapping의 {name}과 PathVariable의 String name을 매핑 하여 준다.
