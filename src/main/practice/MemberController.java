...
public class MemberController {
    ...
    /**회원가입 메서드를 추가하기 앞서 @Autowired를 통해서 MemberService.java 가      
        MemberController.java에 자동 주입되도록 코드를 추가한다.*/
    ...
    public String joinPOST(MemberVO member) throws Exception {
        ...
        member.setMemberPw(encdoPw);            // 인코딩되니 비밀번호 member 객체에 다시 저장
    ...
    // 아이디 중복 검사
    /** MemberController.java에 아이디 중복검사 메서드(memberIdChkPOST)를 작성
     ...
     */
     ...
     // 이메일 인증
     @RequestMapping(value = "/mailChek", method = RequestMethod.GET)
     @ResponseBody
     ...
     * MemberController.java에 로그인 기능을 수행하는 메서드를 작성한다. 파라미터로는
        MemberVO, HttpSerlvetRequest, RedirectAttribute를 사용한다.
    ...
    public String loginPOST(HttpServletRequest request, MemberVO member, RedirectAttribute rttr) throws Exception{
        ...
        // memberLogin() 메서드를 호출하여 위에서 수정한 쿼리가 실행이 되도록 합니다. 실행된 결과로 반환받은 MemberVO인스턴스의 주소를 MemberVO타입의 lvo 변수에 저장한다.
        ...
        if(lvo != null) {
             ...
             */
             ...
             if(true == pwEncoder.matches(rawPw, encodePw)) {   // 비밀번호 일치여부 판단
                ...
                /**
                 * 조건문이 true인 겨우 실행되는 if 구현부에는 로그인 성공 시 실행되어야 할 코드를 작성
        ...
        /**
         * 1.두 가지의 변수를 및 초기화를 진행할 것이다. 먼저 session을 사용하기 위해 session 변수를 선언하고
                return.getSession()으로 초기화한다.
           2.두 번째 변수로 MemberVO 타입의 변수 lvo를 선언하고 이전 포스팅에서 만든
                MemberService.java의 memberLogin메서드로 초기화 한다.
                -인자는 서버로부터 전달받은 memberVo객체를 반환 받아서 변수 lvo에 저장되게 된다.
         ...
         */
          ...
          * lvo 값은 아이디, 비밀번호가 존재할 경우 memberId, memberName, adminCk, money, point 데이터가 담긴 MemberVO
            객체가 저장되게 된다. 아이디, 비밀번호가 존재하지 않을 경우 lvo 값은 null이 저장된다. 따라서 이러한 상황을 이용해서
            if문을 통해 lvo 값이 null일 경우 로그인 실패이기 때문에 로그인 페이지로 리다이렉트 되도록, null이 아닌 경우
            로그인 성공이기 때문에 메인 페이지로 리다이렉트 되도록 return 값을 설정해준다. 

            -return문으로 가기 전 실패일 경우 RedirectAttribute에 실패를 의미하는 데이터를, 성공일 경우 session에 변수 lvo에
...
public String logoutMainGET(HttpServletRequest request) throws Exception {
    ...
    // 세션을 제거하는 작업을 해야 하기 떄문에 HttpSession타입의 session 변수 및 초기화를 한다.
    HttpSession session = request.getSession();

        /** HttpSession
         * 둘 이상의 페이지 요청 또는 웹 사이트 방문에서 사용자를 식별하고 해당 사용자에 대한 정보를 저장하는 방법을 제공
         서블릿 컨테이너는 이 인터페이스를 사용하여 HTTP 클라이언트와 HTTP 서버 간의 세션을 생성한다.
        세션은 사용자의 둘 이상의 연결 또는 페이지 요청에 지정된 기간 동안 지속된다. 세션은 일반적으로 사이트를 여러 번
        방문할 수 있는 한 사용자에 해당한다. 서버는 쿠키를 사용하거나 URL을 다시 작성하는 등 다양한 방법으로 세션을 유지할 수 있다.          
        */

    /**
     * 우리는 사용자의 정보를 "meber"라고 네이밍 한 session을 삭제해야 한다. HttpSession api문서를 보면 우리 목적에 맞는 session을 제거할 수 있는
       ...
       로그아웃을 할 경우 사용될 세션이 없기 때문에 저는 "invalidate()" 메서드를 사용하였다. 아래와 같이 sessiom을 제거하는 코드와
 ...
 * MemberController.java에 뷰(View)에서 요청하는 url의 메서드를 작성한다. 주의할 점은 ajax를 통해서 서버에 요청을 하는 방식이기 때문에
   해당 메서드에 반드시 @ResponseBody 어노테이션을 붙여주어야 한다. 뷰(view)에 특정 데이터를 전달해야 할 경우 변환 방식을 String으로 하면
   되지만 메서드의 작업만 수행하면 되기 때문에 메서드와 반환 방식을 void로 하였다. 메서드 구현부는 저번 포스팅에서 로그아웃 메서드 구현부와 
 ...
 */
 ...
 @ResponseBody // ajax를 통해서 서버에 요청을 하는 방식이기 떄문에 선언
 ...
 /**
  * 로그아웃을 POST방식으로 요청하는 ajax 코드를 아래와 같이 작성한다. 로그아웃을 요청하는 urll 메서드는 다음 순서에서 작성한다.
    [success] 옵션에서 해당 요청이 정상적으로 수행되었는지 확인하기 위해 alert 창이 뜨도록 작성하였고, 현재의 페이지가 새로고침 되도록
    작성하였다. (새로고침을 해주어야 세션의 변경사항이 화면에 반영이 되기 때문이다.)
